...
# Portions copied/adapted from GVHMR hmr4d/utils/smplx_utils.py.
# Runtime paths are rewritten to be self-contained in the SplineHMR repository.
PROJ_ROOT = Path(__file__).resolve().parents[2]
BODY_MODEL_ROOT = PROJ_ROOT / "assets" / "body_models"
...
def make_smplx(type="neu_fullpose", **kwargs):
    if type == "neu_fullpose":
        model = smplx.create(
            model_path=BODY_MODEL_ROOT / "smplx" / "SMPLX_NEUTRAL.npz", use_pca=False, flat_hand_mean=True, **kwargs
        )
        ...
        model = BodyModelSMPLH(model_path=BODY_MODEL_ROOT, **bm_kwargs)
...
def load_parents(npz_path="models/smplx/SMPLX_NEUTRAL.npz"):
    smplx_struct = np.load(BODY_MODEL_ROOT / "smplx" / "SMPLX_NEUTRAL.npz", allow_pickle=True)
