...
def qbetween(v0, v1):
    ...
    c = torch.cross(v0, v1, dim=-1)
    d = (v0 * v1).sum(dim=-1, keepdim=True)
    q = torch.cat([1.0 + d, c], dim=-1)
    fallback = torch.zeros_like(q)
    fallback[..., 1] = 1.0
    q = torch.where((q[..., :1].abs() < 1e-8), fallback, q)
...
class _MatrixCompat:
    @staticmethod
    def get_TRS(rot, pos):
        T = torch.eye(4, device=rot.device, dtype=rot.dtype).expand(rot.shape[0], 4, 4).clone()
        T[:, :3, :3] = rot
        T[:, :3, 3] = pos
        return T

    @staticmethod
    def get_position_from(points, T):
        return torch.einsum("...ij,...nj->...ni", T[..., :3, :3], points) + T[..., None, :3, 3]
    ...
    @staticmethod
    def get_relative_position_to(points, T):
        R = T[..., :3, :3]
        t = T[..., :3, 3]
        return torch.einsum("...ji,...nj->...ni", R, points - t[..., None, :])


matrix = _MatrixCompat()
